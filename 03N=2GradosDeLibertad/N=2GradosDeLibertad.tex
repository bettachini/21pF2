...
Unos dispuestos verticalmente, de una longitud natural $\ell_0 <  L/2$ y constante elástica $k_1$.
Oros dispuestos horizontalmente, de $\ell_0= 0$ (\emph{slinkies}) y constante $k_2$.
...
De dos péndulos de igual longitud $l$ penden pesos de masa diferente $m_a$ y $m_b$.
Estos están acoplados entre sí mediante un resorte de constante elástica $k$.
\begin{enumerate}
	\item Escriba las ecuaciones de movimiento de cada masa. considerando pequeñas oscilaciones.
	En base a lo que se muestra en la figura, ¿cómo se relaciona con que el resorte tenga $\ell_0 \neq 0$?
...
%   \includegraphics[width=\textwidth]{ej1-7}
	\begingroup
		\tikzset{every picture/.style={scale=0.4}}%
		\input{\detokenize{figuras/tikzPéndulosAcoplados}}
	\endgroup
\end{minipage}
